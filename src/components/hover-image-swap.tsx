		...
		<div className={cn('flex gap-3 lg:gap-4', isVertical ? 'flex-row' : 'flex-col', className)}>
			{rows.map((row, rowIdx) => {
				...
				if (row.slots.length === 1) {
					...
					return (
						<div key={rowIdx} className={cn('overflow-hidden rounded-3xl', height, width, row.cardStyle && 'border border-secondary shadow-card')}>
							<img
								src={slot.src}
								alt={slot.alt ?? ''}
								decoding="async"
							...
							return (
								<button
									...
									className={cn(
										...
										isActive
											? cn('bg-transparent grow shrink basis-0 rounded-3xl', slot.href ? 'cursor-pointer' : 'cursor-default', row.cardStyle && 'border border-secondary shadow-card')
